...
static int base64_decode(const char *data, size_t data_len, std::string &raw) {
  ...
  int outlen = EVP_DecodeBlock(reinterpret_cast<uint8_t *>(const_cast<char *>(raw.data())),
...
static int base64_encode(const void *data, size_t data_len, std::string &base64_buffer) {
  ...
  base64_buffer.resize(outlen);
  ret = mbedtls_base64_encode(reinterpret_cast<uint8_t *>(const_cast<char *>(base64_buffer.data())),
...
static int base64_decode(const char *data, size_t data_len, std::string &raw) {
  size_t outlen = 0;
  std::string base64 = base64_strip(data, data_len);
  int ret = mbedtls_base64_decode(NULL, 0, &outlen, reinterpret_cast<const uint8_t *>(base64.data()), base64.size());
  if (ret != MBEDTLS_ERR_BASE64_BUFFER_TOO_SMALL) return ret;
  raw.resize(outlen);
  ret = mbedtls_base64_decode(reinterpret_cast<uint8_t *>(const_cast<char *>(raw.data())), raw.size(), &outlen,
                              reinterpret_cast<const uint8_t *>(base64.data()), base64.size());
  return ret;
}
#endif  // USE_OPENSSL
...
static int compress(const void *data, size_t data_len, std::string &compressed) {
  ...
  int ret = compress2(reinterpret_cast<Bytef *>(const_cast<char *>(compressed.data())), &len,
  ...
  len = compressed.size();
  ret = compress2(reinterpret_cast<Bytef *>(const_cast<char *>(compressed.data())), &len,
                  reinterpret_cast<const Bytef *>(data), data_len, Z_BEST_SPEED);
...
static std::string __hmacsha256(uint32_t sdkappid, const std::string &identifier, uint64_t init_time, uint64_t expire,
  ...
  HMAC(EVP_sha256(), key.data(), key.length(), reinterpret_cast<const unsigned char *>(raw_content_to_be_signed.data()),
  ...
  mbedtls_md_hmac_update(&ctx, reinterpret_cast<const unsigned char *>(raw_content_to_be_signed.data()),
...
TLS_API std::string gen_userbuf(const std::string &account, uint32_t dwSdkappid, uint32_t dwAuthID, uint32_t dwExpTime,
  ...
  int offset = 0;
  if (roomStr.length() > 0) length += 2 + roomStr.length();
  char userBuf[length];
...
TLS_API int genSig(uint32_t sdkappid, const std::string &userid, const std::string &key, const std::string &userbuf,
  ...
  if (userbuf.length() > 0) {
    ...
    base64RawSig = hmacsha256(sdkappid, userid, currTime, expire, key, base64UserBuf);
  } else {
  ...
  if (base64UserBuf.length() > 0) sig_doc.AddMember("TLS.userbuf", base64UserBuf, sig_doc.GetAllocator());
